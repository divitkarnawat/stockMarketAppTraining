...
class ShowChartCompanySector extends Component {
  constructor(props) {
    super(props);
    this.newData = [];
    this.state = {
  ...
  createDataTable(responses) {
    ...
    let cs = this.state.companyName.length;
    for( let i = 0; i<cs; i++)
    {
        responses[i].data.map(stp => {
            this.newData.push([stp.date, this.state.companyName[i], stp.currentPrice]);
        })
    }
    
    for(let i = 0; i<this.state.sectorName.length; i++)
    {
        responses[i+cs].data.map(stp => {
            this.newData.push([stp.date, this.state.sectorName[i], stp.currentPrice]);
        })
    }
    // responses.map((response,id)=>
    // {
    //     response.data.map(stp => {
    //         this.newData.push([stp.date, this.state.sectorName[id], stp.currentPrice]);
    //     })
    // })
  ...
  axios.all([requestOne, requestTwo, requestThree]).then(axios.spread((...responses) => {
    ...
    this.sectorList = responses[1].data;
    this.stockExchangeList = responses[2].data;
 ...
 <FormControl style = {{display: `flex`}}>
        <InputLabel id="sectorName">Sector</InputLabel>
        <Select
        fullWidth
        required
          labelId="sectorName"
          ...
          onChange={(e) => this.handleMultiSelectChange("sectorName",e.target.value)}
          input={<Input id="select-multiple-chip" />}
          renderValue={(selected) => (
            <div >
              {selected.map((value) => (
...
export default ShowChartCompanySector;
